...
from flaskblog.models import User, Question, Grade, IssueReport, Comment, News
...
def home():
	...
	return render_template('home.html', posts=posts, image_file=image_file, title="Home Page")
...
def view_issue(post_id):
	...
	page = request.args.get('page', 1, type=int)
	image_file = current_user.image_file
	return render_template("view_report.html", image_file=image_file, title=post.issuetype, post=post.content)
...
def view_question(post_id):
	...
	if current_user.level == ('Repairman' or 'Administrator'):
		return render_template("studypost.html", title=post.title, user_level=user_level, image_file=image_file, post=post, comments=comments)
	else:
		if form.validate_on_submit():
		...
		return render_template("view_question.html", title=post.title, form = form, user_level=user_level, post=post, comments=comments, image_file=image_file, legend="Add your comment here")
