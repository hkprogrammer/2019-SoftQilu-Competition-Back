...
from flaskblog.models import User, Question, Grade, IssueReport, Comment, News, Memory, Club
from flaskblog.forms import RegistrationForm, ClubForm, LoginForm, UpdateAccountForm, NewsClubForm, GradeForm, QuestionForm, ReportIssueForm, RequestResetForm, ResetPasswordForm, QuestionForm, AnswerForm, NewsForm
...
def create_club():
	...
	form = ClubForm()
	if form.validate_on_submit():
		post = Club(clubname=form.clubname.data)
		db.session.add(post)
		db.session.commit()
